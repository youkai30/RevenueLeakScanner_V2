...
def apply_stealth_builtin(page: Page) -> bool:
    ...
    Covers the 5 most important anti-bot detection vectors.
...
def is_retryable_error(exc: Exception) -> bool:
    ...
    retryable_keywords = [
        "timeout", "connection reset", "connection closed", "connection refused",
        "name not resolved", "net::err_connection_reset", "net::err_connection_closed",
        "net::err_connection_refused", "net::err_name_not_resolved", "net::err_timed_out",
        "network error", "load state",
    ]
    ...
    try:
        page.evaluate("""() => {
            const cookieSelectors = [
                '#onetrust-accept-btn-handler', '.cookie-accept', '.js-accept-cookie',
                '#btn-accept-cookies', '#accept-cookies',
                '[id*="cookie" i] button[class*="accept" i]',
                '[class*="cookie" i] button[class*="accept" i]',
                '#shopify-pc-btn-accept', '.accept-cookies-button', '#accept-all-cookies'
            ];
            ...
            const closeSelectors = [
                'button[aria-label*="close" i]', '[class*="close-modal" i]',
                '[class*="modal-close" i]', '.modal-close', '.close-modal', '.close-btn',
                'button.close', '[class*="close" i] button',
                '[class*="Newsletter" i] [class*="close" i]',
                '.klaviyo-form button[tabindex="0"]:not([type="submit"])',
                '.klaviyo-form [class*="close" i]', '.rego-close-button',
                '.newsletter-close', '.close-icon', '.icon-close', '.popup-close', '#popup-close'
            ];
